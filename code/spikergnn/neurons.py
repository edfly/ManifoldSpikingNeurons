...
class ManifoldLIFNeuron(nn.Module):
    ...
    def __init__(
        ...
        Args:
            ...
            input_mag_cap: Absolute upper bound on per-step input magnitude
                (default 0.6). Fixed constant, deliberately NOT scaled by
                u_th: coupling the cap to the threshold would pin the firing
                period at 2 steps (rate 0.5) regardless of u_th, giving the
                learnable threshold zero control over the firing rate.
                With a fixed cap, u_th modulates the accumulation period and
                hence the rate (u_th=1.0 -> ~0.5, u_th=2.0 -> ~0.25 at
                steady state with alpha=0.9).
        """
        super().__init__()
        ...
        # Spike count tracking. Counters ACCUMULATE across forward calls and
        # are reset only by reset_membrane(). Callers that invoke the neuron
        # once per episode (full-T input) should call reset_membrane() before
        # each episode; callers that step with T=1 (e.g. STMP) get the true
        # episode-level rate by reading the counters after the last step,
        # instead of the last step's single-step rate (measurement artifact
        # fixed: previously the counters were reset at the start of every
        # forward call, so _spike_rate reflected only the final T=1 call).
        self._spike_count = 0
        self._total_neurons = 0   # accumulated N*T for spike rate computation
        self._spike_rate = 0.0    # Empirical spike rate (running, since last reset)
        self._surrogate_spike_sum = None  # differentiable spike sum (surrogate)
    ...
    def forward(
        ...
        for t in range(T):
            ...
            # Spike rate control: clamp per-step input magnitude to an
            # ABSOLUTE cap (default 0.6), independent of the threshold.
            # This guarantees the membrane potential cannot exceed the
            # (initial) threshold in a single step — it must accumulate over
            # 2+ steps via alpha decay, giving sr <= ~0.5 at steady state —
            # while leaving the firing period, and hence the rate, under the
            # control of the learnable threshold u_th. (An earlier version
            # scaled the cap with u_th; that made the firing period a
            # structural constant of (alpha, cap_factor) and nullified
            # u_th's control over the rate, breaking L_sr's mechanism.)
            # With the default cap=0.6 and alpha=0.9:
            #   u_th = 1.0: fire every ~2 steps (rate ~0.5, near r*=0.4)
            #   u_th = 2.0: fire every ~4 steps (rate ~0.25)
            w_input_mag = torch.clamp(w_input_mag, max=self.input_mag_cap)

            u_mag = self.alpha * u_mag + w_input_mag

            # Effective (clamped) learnable threshold, used by the membrane
            # clamp below and the spike trigger below.
            effective_threshold = self.u_th_learnable.clamp(min=0.05)

            # ── Membrane potential clamp ───────────────────────────
            # Prevent runaway excitation: limit u_mag to 1.5x the effective
            # threshold. With the absolute input cap and firing + hard reset,
            # u_mag cycles between 0 and ~1.14*u_th, so 1.5x is sufficient.
            ...
            # Accumulates across forward calls until reset_membrane() is called.
        ...
        # Accumulate counters (running totals since last reset_membrane())
    ...
    def reset_membrane(self):
        """Reset the persistent membrane potential and all spike counters.

        Call this once per episode (before the first forward call of the
        episode), regardless of whether the episode is processed as a single
        full-T call or as a sequence of T=1 calls.
        """
        self._u_mag = None
        self._spike_count = 0
    ...
    def spike_rate_surrogate(self) -> torch.Tensor:
        """Return the differentiable (surrogate) spike rate since the last reset.

        This uses the surrogate gradient output (binary in forward, smooth in
        backward), making it differentiable with respect to u_th_learnable
        for spike rate regularization. The value accumulates across forward
        calls until reset_membrane() is called, so it reports the episode
        rate for both full-T and T=1-step callers.
